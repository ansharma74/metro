...
'''
Filename: plot_sv150_t_bubble2_cir2.py
...
Usage example:
     ./plot_sv150_t_bubble2_cir2.py

Output:
     Output file: output/sv_top150_cir2.png
...
for rows in cmpy:
    if int(rows["Rank"]) < ranks:
       x.append(rows["Rank"])
       theta.append(2 * np.pi * int(rows["Rank"])/7)
       point.append(ranks - int(rows["Rank"]))
...
for ptr in point: 
    if ptr < 11:
        ax.annotate(z[ranks - ptr -1], xy=(point[ranks - ptr-1]+.05, theta[ranks - ptr-1]))
        ax.annotate(int(y[ranks - ptr -1]), xy=(point[ranks - ptr-1]-.05, theta[ranks - ptr-1]))
