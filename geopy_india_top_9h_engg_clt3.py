...
#City Database file
city_file = "datafiles/A1000_eng_1r.csv"
...
zone_info={}
state_zone_info={}
cmap = plt.cm.Set3 # use 'hot' colormap
...
for rows in cities:
    ...
    if rows["Rank"]:
        l_rank = int(rows["Rank"])
    else:
        l_rank = 0
    print(l_rank)
    loc = geolocator.geocode(l_city)
    x, y = map(loc.longitude, loc.latitude)
    if int(l_rank) > 0 and int(l_rank) <= 50:
        map.plot(x,y,marker='*',color='#00acdb',markersize=scale, markeredgecolor='k')
    elif int(l_rank) > 50 and int(l_rank) <= 100:
        map.plot(x,y,marker='D',color='#0dacdb',markersize=scale, markeredgecolor='y')
    elif int(l_rank) > 100 and int(l_rank) <= 200:
        map.plot(x,y,marker='^',color='#00aedb',markersize=scale, markeredgecolor='m')
    else:
        map.plot(x,y,marker='o',color='#00fcdb',markersize=scale, markeredgecolor='b')

    print(fg('orchid') + "MAP: Cities:",rows["S.No."],l_city)
...
print("########## Legends #####################")
cream_square = mlines.Line2D([], [], color='orange', marker='s', linestyle='None',
                          markersize=5, label='State-Zones (Colored areas) ')
cyan_star = mlines.Line2D([], [], color='orange', marker='*', linestyle='None',
                          markersize=5, label='Top 50 Engg Colleges')
cyan_cmn = mlines.Line2D([], [], color='orange', marker='D', linestyle='None',
                          markersize=5, label='Top 50 to 100 Ranked Engg Colleges')
cyan_crt = mlines.Line2D([], [], color='orange', marker='^', linestyle='None',
                          markersize=5, label='Top 100 to 200 Ranked Engg Colleges')
cyan_crl = mlines.Line2D([], [], color='orange', marker='o', linestyle='None',
                          markersize=5, label='Other Engg College Towns')
legend1 = plt.legend(handles=[cream_square, cyan_star, cyan_cmn, cyan_crt, cyan_crl], 
        title='900+ Enguneering Colleges', loc='upper right' , facecolor = 'c', fontsize=5)

legend1.get_title().set_fontsize('6')
plt.title('India: Engineering Colleges', size = 7)

plt.savefig('output/ind_eng900.png', dpi=1200)
